...
TECH_COMMAND_DIR=$(command -v tech >/dev/null 2>&1 && dirname "$(which -a tech | grep '/usr/local/bin/tech' | head -1)" || { [ -x /usr/local/bin/tech ] && echo "/usr/local/bin" || echo "$PATH" | cut -d ':' -f1; })
...
SERVICE_FILE="$USER_DIR/opt/systemd/system/autostart.service"
SERVICE_NAME="autostart.service"
CONFIG_DIR_SWAP="$USER_DIR/opt/tech-scripts"
...
BASIC_DIRECTORY="$USER_DIR/tmp/tech-scripts $USER_DIR/opt/tech-scripts $USER_DIR/usr/local/tech-scripts $TECH_COMMAND_DIR/tech"
